...
func VerifyOrCreateSyncTarget(client *clientset.Clientset, ctx context.Context, locationName string, labels []string) error {
	logger := klog.FromContext(ctx)

	// Get the SyncTarget object
	syncTarget, err := client.EdgeV2alpha1().SyncTargets().Get(ctx, locationName, metav1.GetOptions{})
	if err == nil {
		logger.Info(fmt.Sprintf("Found SyncTarget %s", locationName))
		// Check that SyncTarget has an "id" label matching locationName
		err = VerifySyncTargetId(syncTarget, client, ctx, locationName)
		if err != nil {
			return err
		}
		// Check that SyncTarget has user provided key=value pairs, add them if not
		err = VerifySyncTargetLabels(syncTarget, client, ctx, locationName, labels)
		return err
	} else if !apierrors.IsNotFound(err) {
		// Some error other than a non-existant SyncTarget
		logger.Error(err, fmt.Sprintf("Problem checking for SyncTarget %s", locationName))
		return err
	}
	// SyncTarget does not exist, must create
	logger.Info(fmt.Sprintf("No SyncTarget %s, creating it", locationName))
	...
	if err != nil {
		logger.Error(err, fmt.Sprintf("Failed to create SyncTarget %s", locationName))
		return err
	}
...
func VerifySyncTargetId(syncTarget *v2alpha1.SyncTarget, client *clientset.Clientset, ctx context.Context, locationName string) error {
	logger := klog.FromContext(ctx)
	...
	if err != nil {
		logger.Error(err, fmt.Sprintf("Failed to update SyncTarget %s", locationName))
		return err
	}
...
// Check that SyncTarget has user provided key=value pairs, add them if not
func VerifySyncTargetLabels(syncTarget *v2alpha1.SyncTarget, client *clientset.Clientset, ctx context.Context, locationName string, labels []string) error {
	logger := klog.FromContext(ctx)
...
func VerifyOrCreateLocation(client *clientset.Clientset, ctx context.Context, locationName string, labels []string) error {
	logger := klog.FromContext(ctx)

	// Get the Location object
	location, err := client.EdgeV2alpha1().Locations().Get(ctx, locationName, metav1.GetOptions{})
	if err == nil {
		logger.Info(fmt.Sprintf("Found Location %s", locationName))
		// Check that Location has user provided key=value pairs, add them if not
		err = VerifyLocationLabels(location, client, ctx, locationName, labels)
		return err
	} else if !apierrors.IsNotFound(err) {
		// Some error other than a non-existant SyncTarget
		logger.Error(err, fmt.Sprintf("Problem checking for Location %s", locationName))
		return err
	}
	// Location does not exist, must create
	logger.Info(fmt.Sprintf("No Location %s, creating it", locationName))
	...
	if err != nil {
		logger.Error(err, fmt.Sprintf("Failed to create Location %s", locationName))
		return err
	}
...
// Check that Location has user provided key=value pairs, add them if not
func VerifyLocationLabels(location *v2alpha1.Location, client *clientset.Clientset, ctx context.Context, locationName string, labels []string) error {
	logger := klog.FromContext(ctx)
	...
	if updateLocation {
		...
		if err != nil {
			logger.Error(err, fmt.Sprintf("Failed to update Location %s", locationName))
			return err
		}
	}
	return nil
